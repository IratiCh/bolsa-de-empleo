...
function AsignarOferta() {
    ...
    const handleAsignar = async (idDemandante, externo = "") => {
        try {
            ...
            const response = await fetch(
                `/api/empresa/asignar_oferta/${id}/asignar`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                ...
                <div className="OFERTAS">
                    ...
                    {oferta && (
                        <>
                            ...
                            {oferta.abierta !== 0 && (
                                <div className="error">
                                    La oferta está cerrada. Solo puedes
                                ...
                                <tbody>
                                    {inscritos.length > 0 ? (
                                        inscritos.map((demandante) => {
                                            const cvData =
                                                cvByDemandante[demandante.id];
                                            const showFormInline =
                                                cvExpanded[demandante.id] &&
                                                hasCvForm(cvData);
                                            return (
                                                <React.Fragment
                                                    key={`inscrito-${demandante.id}`}
                                                >
                                                    <tr>
                                                        <td>
                                                            {
                                                                demandante.nombre_completo
                                                            }
                                                        </td>
                                                        <td>
                                                            {demandante.email}
