...
class OfertaController extends Controller
{
     ...
     **/
    public function cerrarOferta(Request $request, $id)
    {
        $request->validate([
        ...
        try {
            ...
            $oferta->motivo_cierre = $request->motivo; // Motivo de cierre
    ...
    {
        try {
            $oferta = Oferta::with('titulos')->findOrFail($id);
    ...
    public function modificarOferta(Request $request, $id)
    {
        try {
            ...
            // Actualizar títulos (relación many-to-many)
            if ($request->has('titulos')) {
                $oferta->titulos()->sync($request->titulos);
            }
